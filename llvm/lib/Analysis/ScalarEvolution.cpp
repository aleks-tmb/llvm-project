...
void ScalarEvolution::LoopGuards::collectFromBlock(
  ...
  auto CollectCondition = [&](ICmpInst::Predicate Predicate, const SCEV *LHS,
    ...
    auto TryTightenYFromZExtAddUGE = [&](const SCEV *From,
      ...
      // C < 0 needs Y >=u |C| so narrow wrap doesn't fake-satisfy the
      // guard; C >= 0 has no such precondition (positive-C wrap is
      // ruled out by YMin > 0 below).
      if (C->isNegative() &&
          !SE.isKnownPredicateViaConstantRanges(
              CmpInst::ICMP_UGE, YRewritten, SE.getConstant(-*C)))
        return false;
